...
#### Что это дало на измеренном прогоне

Тот же предмет, тот же бриф, до и после того, как правила гейта стали доезжать до шага:

| | было | стало |
|---|---|---|
| раундов правки | 3 | 3 |
| замечаний по раундам | 4, 4, 3 | 6, 2, 2 |
| замечаний про объём | в каждом раунде | ни одного |
| проза против брифа 8–12 тысяч | ушла за потолок | 11 977 / 11 995 / 11 275 |

Замечание про объём в прошлом прогоне выглядело так: критик называл на глаз 18–20 тысяч, потом
13 тысяч, потом 13,5 тысяч для одного и того же текста и дважды просил кого-нибудь посчитать
по-настоящему. Гейт закрыл это внутри шага писателя — 13 491 → 12 456 → 11 977, каждый раз по
арифметике из сообщения, — и все шесть замечаний первого раунда ушли на объяснение механизма.

Стилист после этого нашёл механический слой чистым полностью: ноль дефисов вместо тире, ноль
прямых кавычек, ноль мёртвых фраз, ноль абзацев, открытых связкой. Это `forbid_file`,
исполненный на писателе; стилисту осталось то, куда регулярка не достаёт, — ритм, обращение,
терминология.
