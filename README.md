...
- Результат в одном месте. Конвейер объявляет, что он выдаёт и под какими именами, и движок
  собирает это в `result/` каталога прогона — с раскладкой, которую артефакты друг другу
  обещали: статья рядом с каталогом своих картинок. Плюс зеркало последней выдачи в
  `latest/` корня проекта, чтобы не искать её на четыре уровня вглубь под именем-меткой
  времени; выдача каждого прогона при этом остаётся своей и не перезаписывается.
- Петля говорит, чего не закрыла. Замечания, которые раунды не сняли, движок пишет в
  `_unresolved.md` из типизированного вердикта и кладёт рядом с результатом. И перестаёт
...
событий и судит результат по файлам в `result/`.
...
`_unresolved.md` с поимённо названными открытыми пунктами, и доставка кладёт его рядом с
...
├── result/                     # ВЫДАЧА: то, за чем прогон запускали
│   ├── article.md
│   ├── figures/
│   └── _unresolved.md          # что петля не закрыла, если такое осталось
└── steps/
    ├── scan/main/output/
...
запись описания, поиск источников в сети, остановки на выверку, веб-интерфейс. Проверки: 701
тест, строгая типизация, линтер.
